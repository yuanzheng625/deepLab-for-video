...
map_dir0 = dir(fullfile(map_folder, '*.bin.0'));
...
for i = 1 : numel(map_dir0)
    fprintf(1, 'processing %d (%d)...\n', i, numel(map_dir0));
    map = LoadBinFile(fullfile(map_folder, map_dir0(i).name), 'int16');

    img_fn = map_dir0(i).name(1:end-4);
    imwrite(uint8(map), colormap, fullfile(save_result_folder, [img_fn, '_0.png']));
end
